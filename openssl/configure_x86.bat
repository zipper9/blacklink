...
perl Configure VC-WIN32 no-makedepend no-shared no-engine no-static-engine no-mdc2 no-ec2m no-sm2 no-sm4 no-aria no-camellia no-cast no-gost no-psk no-srp no-hw-padlock no-tests no-legacy --prefix=\tmp --openssldir=\tmp
if errorlevel 1 goto :configure_error

set ASMFILES=crypto\aes\aes-586.S crypto\aes\aesni-x86.S crypto\aes\vpaes-x86.S crypto\bf\bf-586.S crypto\bn\bn-586.S crypto\bn\co-586.S crypto\bn\x86-gf2m.S crypto\bn\x86-mont.S crypto\chacha\chacha-x86.S crypto\des\crypt586.S crypto\des\des-586.S crypto\ec\ecp_nistz256-x86.S crypto\md5\md5-586.S crypto\modes\ghash-x86.S crypto\poly1305\poly1305-x86.S crypto\rc4\rc4-586.S crypto\ripemd\rmd-586.S crypto\sha\sha1-586.S crypto\sha\sha256-586.S crypto\sha\sha512-586.S crypto\whrlpool\wp-mmx.S crypto\x86cpuid.S
del %ASMFILES% >NUL
nmake %ASMFILES%
...
for %%L in (%ASMFILES% %CONFFILES% %GENFILES%) do (
 dos2unix -q %%L
)
...
sed "$adepend:\n\n$(OUTDIR):\n\t-mkdir ..\\\\vc16\n\t-mkdir ..\\\\vc16\\\\Win32\n\t-mkdir ..\\\\vc16\\\\Win32\\\\$(CONFIG)\n\t-mkdir $(OUTDIR)\n\nlibclean:\n\t-del /Q /F $(LIBS) ossl_static.pdb $(OUTDIR)\\\\*.obj $(OUTDIR)\\\\*.asm\n\nclean: libclean\n\n" -i %F%
