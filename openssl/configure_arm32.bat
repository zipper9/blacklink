...
perl Configure VC-WIN32-ARM no-makedepend no-shared no-engine no-static-engine no-mdc2 no-ec2m no-sm2 no-sm4 no-aria no-camellia no-cast no-gost no-psk no-srp no-hw-padlock no-tests no-legacy --prefix=\tmp --openssldir=\tmp
