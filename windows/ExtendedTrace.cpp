...
static const int LIBCOUNT   = 47;

#define LIT(x) x, (sizeof(x)-1)

// FIXME: Do we really need all this shit ???
...
static void checkBuggyLibrary(PCSTR library)
{
	for (int i = 0; i < LIBCOUNT; i++)
	{
		...
		if (stricmp(library, lib) == 0)
		{
			...
			_sntprintf(&buf[0], buf.length(), crashMessage.c_str(), app.c_str());
		
			MessageBox(NULL, buf.c_str(), _T("Unhandled exception"), MB_OK | MB_ICONERROR);
			ExitProcess(1);
		}
	}
}
...
static void InitSymbolPath(PSTR lpszSymbolPath, PCSTR lpszIniPath)
{
	...
	if (GetEnvironmentVariableA("_NT_SYMBOL_PATH", lpszPath, BUFSIZE))
	{
	...
	if (GetEnvironmentVariableA("_NT_ALTERNATE_SYMBOL_PATH", lpszPath, BUFSIZE))
	{
		strcat(lpszSymbolPath, ";");
		strcat(lpszSymbolPath, lpszPath);
	}
	...
	if (GetEnvironmentVariableA("SYSTEMROOT", lpszPath, BUFSIZE))
	{
		strcat(lpszSymbolPath, ";");
		strcat(lpszSymbolPath, lpszPath);
		strcat(lpszSymbolPath, ";");
...
#endif
{
	...
	else _tcscpy(lpszModule, _T("?")); // Not found :(
...
static BOOL GetFunctionInfoFromAddresses(DWORD64 fnAddress, DWORD64 stackAddress, LPTSTR lpszSymbol)
{
	BOOL              ret = FALSE;
	DWORD64           dwDisp = 0;
	TCHAR             lpszUnDSymbol[BUFSIZE]=_T("?");
	CHAR              lpszNonUnicodeUnDSymbol[BUFSIZE]="?";
	LPTSTR            lpszParamSep = NULL;
	LPCTSTR           lpszParsed = lpszUnDSymbol;
	BYTE              symBuf[sizeof(SYMBOL_INFO) + BUFSIZE];
	PSYMBOL_INFO      pSym;

	::ZeroMemory(symBuf, sizeof(SYMBOL_INFO) + BUFSIZE);
	pSym = (PSYMBOL_INFO)symBuf;

	pSym->SizeOfStruct = sizeof(SYMBOL_INFO);
	pSym->MaxNameLen = BUFSIZE;
	...
	if (SymFromAddr(GetCurrentProcess(), fnAddress, &dwDisp, pSym))
	{
		...
		UnDecorateSymbolName(pSym->Name, lpszNonUnicodeUnDSymbol, BUFSIZE, 
			UNDNAME_COMPLETE | 
			UNDNAME_NO_THISTYPE |
...
#endif
{
	...
	DWORD          dwDisp;
	TCHAR          lpszFileName[BUFSIZE] = _T("");
	TCHAR          lpModuleInfo[BUFSIZE] = _T("");
...
void StackTrace(HANDLE hThread, File& f, const PCONTEXT pCtx)
{
	...
	BOOL           bResult;
	TCHAR          symInfo[BUFSIZE] = _T("?");
	TCHAR          srcInfo[BUFSIZE] = _T("?");
