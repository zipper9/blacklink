	...
	switch (cd->nmcd.dwDrawStage)
	{
		...
		case CDDS_SUBITEM | CDDS_ITEMPREPAINT:
		{
			...
			if (colIndex == COLUMN_STATUS)
			{
				...
				int shift = 0;
				if (ii->status == ItemInfo::STATUS_RUNNING || ii->isTorrent)
				{
					...
					else if (BOOLSETTING(STEALTHY_STYLE_ICO) || ii->m_is_force_passive)
					{
						...
						else if (ii->status == ItemInfo::STATUS_RUNNING)
						{
							...
							if (!ii->isTorrent) // TODO - показывать скорость и для торрентов
							{
								CImageList& images = HLS_S(hls > 30) || HLS_L(hls) < 70 ? m_speedImages : m_speedImagesBW;
			...
			if (i->parent != nullptr || children.size() <= 1)
			{
				switch (SETTING(TRANSFERLIST_DBLCLICK))
				{
					...
					case 5: // !SMT!-UI					
						...
						ClientManager::getInstance()->connect(i->hintedUser, Util::toString(Util::rand()), false, l_is_active_client); // [!] IRainman fix.
						break;
					case 6:
...
TransferView::ItemInfo* TransferView::findItem(const UpdateInfo& ui, int& pos) const
{
	...
	for (int j = 0; j < count; ++j)
	{
		...
		if (ii)
		{
			...
			else if (ui.download == ii->download && !ii->parent && ii->isTorrent == ui.isTorrent)
			{
				if (!ii->isTorrent)
				{
...
void TransferView::onSpeakerAddItem(const UpdateInfo& ui)
{
	...
	if (!ui.isTorrent)
	{
		...
		{
#ifdef FLYLINKDC_USE_DEBUG_TRANSFERS
			LogManager::message("SKIP missing token TRANSFER_ADD_ITEM ErrorStatus: ui.token = " + ui.token);
#endif
			return;
		}
	}
	ItemInfo* ii = new ItemInfo(ui.hintedUser, ui.download, ui.isTorrent);
#ifdef FLYLINKDC_USE_TORRENT
	ii->isSeeding = ui.isSeeding;
	ii->sha1 = ui.sha1;
#endif
#ifdef FLYLINKDC_USE_DEBUG_TRANSFERS
	if (ui.token.empty())
	...
	for (auto i = t.cbegin(); i != t.cend(); ++i)
	{
		switch (i->first)
		{
			...
			case TRANSFER_REMOVE_TOKEN_ITEM:
			{
				...
				for (int j = 0; j < ctrlTransfers.GetItemCount(); ++j)
				{
					...
					if (ui.isTorrent && ii->isTorrent)
						l_is_find |= ii->sha1 == ui.sha1;
					else
					{
						if (!ui.isTorrent && !ii->isTorrent)
						{
							l_is_find |= ii->token == ui.token;
							if (!l_is_find)
							{
								if (
			...
			case TRANSFER_REMOVE_DOWNLOAD_ITEM:
			{
				...
				dcassert(!ui.isTorrent);
				if (!ui.target.empty() && !ui.isTorrent)
				{
#ifdef FLYLINKDC_USE_DEBUG_TRANSFERS
					unsigned l_count_remove = 0;
#endif
					for (int j = 0; j < ctrlTransfers.GetItemCount(); ++j)
					{
						...
						if (ui.download && ii->target == ui.target)
						{
							...
							j = 0;
#ifdef FLYLINKDC_USE_DEBUG_TRANSFERS
							LogManager::message("TRANSFER_REMOVE_DOWNLOAD_ITEM ui.target = " + Text::fromT(ui.target) + " ui.token = " + ui.token);
							l_count_remove++;
#endif
						}
					}
					...
					if (!l_count_remove)
					{
						LogManager::message("[!] Not found TRANSFER_REMOVE_DOWNLOAD_ITEM ii->target = " + Text::fromT(ui.target) + " ui.token = " + ui.token);
					}
#endif
				}
			}
			...
			case TRANSFER_REMOVE_ITEM:
			{
				...
				else
				{
					...
					LogManager::message("[!] TRANSFER_REMOVE_ITEM not found ErrorStatus: " + Text::fromT(ui.errorStatusString) +
					                    " Status = " + Text::fromT(ui.statusString) + " ui.token = " + ui.token);
#endif
					//dcassert(0);
				}
			}
			...
			case TRANSFER_UPDATE_ITEM:
			{
					...
					if (ui.download && !ui.isTorrent)
					{
						const ItemInfo* parent = ii->parent ? ii->parent : ii;
						{
							if (!ui.token.empty())
							{
								...
								{
#ifdef FLYLINKDC_USE_DEBUG_TRANSFERS
									LogManager::message("SKIP missing token TRANSFER_UPDATE_ITEM  ui.token = " + ui.token);
#endif
						...
						changeParent = !ui.isTorrent && !ii->isTorrent && (ui.updateMask & UpdateInfo::MASK_FILE) && ui.target != ii->target;
						if (changeParent)
						{
				...
				else
				{
					...
					{
						LogManager::message("[!] TRANSFER_UPDATE_ITEM not found error User = " + ui.hintedUser.user->getLastNick() + " status: " +
						                    Text::fromT(ui.errorStatusString) + " Status = " + Text::fromT(ui.statusString) +
						                    " info = " + ui.hintedUser.user->getLastNick() + "is download = " + Util::toString(ui.download) + " ui.token = " + ui.token);
					}
					else
					{
						LogManager::message("[!] TRANSFER_UPDATE_ITEM not found error status: " +
						                    Text::fromT(ui.errorStatusString) + " Status = " + Text::fromT(ui.statusString) +
						                    "is download = " + Util::toString(ui.download) + " ui.token = " + ui.token);
					}
#endif
					// dcassert(0);
					//if(!ui.token.empty() || ui.isTorrent) // Дублируются
					//if (ui.isTorrent && !ui.sha1.is_all_zeros() || (!ui.token.empty() && !ui.isTorrent)) // Лучший вариант
					{
						onSpeakerAddItem(ui);
			...
			case TRANSFER_UPDATE_PARENT:
			{
				...
				if (ui.hintedUser.user || ui.isTorrent)
				{
					int pos = -1;
					ItemInfo* ii = findItem(ui, pos);
					if (ii)
...
const tstring TransferView::ItemInfo::getText(uint8_t col) const
{
	...
	switch (col)
	{
		case COLUMN_USER:
			if (isTorrent)
				return hits == -1 ? Util::emptyStringT : (Util::toStringW(hits) + _T(' ') + TSTRING(FILES));
			return hits == -1 ? nicks : (Util::toStringW(hits) + _T(' ') + TSTRING(USERS));
		case COLUMN_HUB:
			if (isTorrent)
			{
			...
			{
				return hits == -1 ? hubs : (Util::toStringW(running) + _T(' ') + TSTRING(NUMBER_OF_SEGMENTS));
			}
		case COLUMN_STATUS:
		{
		...
		case COLUMN_PATH:
		{
			if (isTorrent) return Util::getFilePath(target);
...
void TransferView::starting(UpdateInfo* ui, const Transfer* t)
{
	...
	const auto token = t->getConnectionQueueToken();
	dcassert(!token.empty());
	ui->setToken(token);
	ui->setIP(t->getIP());
}
...
void TransferView::on(DownloadManagerListener::Status, const UserConnection* conn, const std::string& reason) noexcept
{
	if (!ClientManager::isBeforeShutdown())
	{
...
void TransferView::on(UploadManagerListener::Starting, const UploadPtr& aUpload) noexcept
{
	if (!ClientManager::isBeforeShutdown())
	{
		...
		const auto token = aUpload->getConnectionQueueToken();
		dcassert(!token.empty());
		ui->setToken(token);
...
void TransferView::on(UploadManagerListener::Tick, const UploadArray& ul) noexcept
{
	if (!ClientManager::isBeforeShutdown() && !MainFrame::isAppMinimized())
	{
		for (auto j = ul.cbegin(); j != ul.cend(); ++j)
		{
			...
			UpdateInfo* ui = new UpdateInfo(j->hintedUser, false); // [!] IRainman fix.
			ui->setStatus(ItemInfo::STATUS_RUNNING);
			ui->setActual(j->actual);
			ui->setPos(j->pos);
			ui->setSize(j->size);
...
void TransferView::onTransferComplete(const Transfer* aTransfer, const bool download, const string& aFileName)
{
	...
	if (!ClientManager::isBeforeShutdown())
	{
		...
		*/
		...
		const auto token = aTransfer->getConnectionQueueToken();
		dcassert(!token.empty());
...
void TransferView::PauseSelectedTransfer(void)
{
	...
	if (ii)
	{
		...
		QueueManager::getInstance()->setAutoPriority(target, false);
