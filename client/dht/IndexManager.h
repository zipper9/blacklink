...
namespace dht
{
	...
	{
		File() { };
		File(const TTHValue& tth, int64_t size, bool partial) :
			tth(tth), size(size), partial(partial) { }
	...
	public:
		...
		void setPublish(bool publish) { this->publish = publish; }
